 ...
 * Datumsfeld (F32-Bekannte-Grenze zu `?von=`/`?bis=`: der Filter prüft sein
 * Format nicht selbst) — die drei Werte hier sind die einzige Quelle, die
 * `von` an GET /api/verbrauch übergibt, und liefern immer ein gültiges
 * ISO-8601-Datum mit `Z`-Suffix (Muster `erstellt_am`, jetzt().toISOString()).
 *
 * Wird aufgerufen von:
