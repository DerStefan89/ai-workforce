...
export interface AufrufEingaben {
  ...
  /** F31 WS-3b (Stefan 20.09.2026, MCP-Start): gesetzt, hängt baueAufruf `--strict-mcp-config --mcp-config <Wert>` an — begrenzt für diesen Lauf die in E-187 (`docs/projekt/zielfassung.md` §9.4) benannte Lücke, dass `--tools`/`--allowedTools` MCP-Werkzeuge nicht abdecken (real gemessen, F31 WS-3b: die zwei Account-MCP-Server `claude.ai Claude Docs`/`claude.ai Google Drive` laden trotz `--setting-sources ''` und trotz `--tools Read,Grep,Glob`, sichtbar an `mcp_servers`/acht zusätzlichen `mcp__*`-Werkzeugen im `stream-json`-Init). Ausschließlich vom Jarvis-Chat-Pfad gesetzt (Wert `'{"mcpServers":{}}'`); jeder andere Aufrufer lässt das Feld unbesetzt und bekommt unverändert keine MCP-Begrenzung. scripts/leitstand-server.mjs' pruefeStartauftrag lehnt das Feld im Body von POST /api/laeufe ab (Muster settingSources). */
  mcpConfig?: string
}
