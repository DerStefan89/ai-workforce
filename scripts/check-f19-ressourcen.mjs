 ...
 *
 * Regel 6 trägt sechs eng benannte Ausnahmen (F346_AUSNAHMEN, seit F24 aus
 ...
 * seit F27 WS-1 'scout', seit F26 WS-1 'jarvis', seit F34 WS-1
 * 'product-coach' und seit F39 WS-1 'architekt' erlauben weiterhin
 * 'claude-code', obwohl 'claude-code' STRUCTURED_OUTPUT nicht bereitstellt
 * (kein --output-schema-Mechanismus, F-337) — alle sechs Verengungen wurden
 * geprüft und real verworfen (src/rollen/index.ts, Kopfkommentar, F19 WS-2;
 * für scout siehe F27/feature.md Nicht-Ziele, "kein Codex-Spike in WS-1";
 * für jarvis, product-coach und architekt läuft ein Chat-/Sparring-/
 * Architektur-Lauf über den direkten POST-Pfad strukturell IMMER als worker
 * 'claude-code' bei fehlendem Codex): ein
...
if (befunde.length === befundeVor6) {
  console.log(
    "✓ (6) Jeder in erlaubte_worker genannte, registrierte Worker deckt die benoetigte_capabilities seiner Rolle vollständig (F-346) — außer den sechs benannten Ausnahmen ('router'/'claude-code', 'code-reviewer'/'claude-code', 'scout'/'claude-code', 'jarvis'/'claude-code', 'product-coach'/'claude-code' und 'architekt'/'claude-code', je Capability 'STRUCTURED_OUTPUT')."
  )
}
