...
| F10-Leitstand-Gate | `scripts/check-f10-leitstand.mjs` (noch nicht vorhanden — kommt in F10 WS-1, AK10) | AK3 (Options-Sperre gegen `AusfuehrungsOptionen`-Felder und unbekannte Body-Felder im Startauftrag), AK4 (Loopback-Bindung `127.0.0.1`), AK5 (laufId-Eindeutigkeit, synchrone Reservierung vor `fuehreAufgabeDurch`), AK6 (Pflicht-`.catch()` um den Fire-and-forget-Aufruf, flüchtige `GET /api/startfehler`-Projektion) | (noch offen — Gate entsteht erst in WS-1) | (noch offen — Gate entsteht erst in WS-1) |
