...
|---|---|---|---|---|
| Doku-Gate | `scripts/check-docs.mjs` | tote Verweise, Versionsnummern außerhalb package.json, Frische-Widerspruch in Einzeldokumenten, Frische-Widerspruch zwischen Dokumentenpaaren (Prüfung 4; Paar CHANGELOG → LEARNING-STATE aktiv seit 2026-09-26), Hedging-Wörter ohne Evidenz-Marker in state/Report-Dateien | Testzeile `React v19, siehe \`keine/existierende/datei.md\`` (temporär in CLAUDE.md eingefügt) → 2 Befunde: toter Verweis + Versionsnummer; echter Fund (kein Testfall) nach Erweiterung von Prüfung 1 auf `.claude/skills/*/SKILL.md` und `.claude/commands/*.md`: `.claude/skills/spec-schreiben/SKILL.md:88: Verweis auf \`state/triage.md\` — Datei existiert nicht` | CLAUDE.md:79 `npm run check` → Exit 0 löst keinen Versionsnummer-Befund aus; README.md:34 verweist auf `settings.local.json`, das per .gitignore absichtlich fehlt → kein Befund; nach Behebung von `spec-schreiben/SKILL.md:88` (Verweis auf `state/tasks/` umgebogen) → `npm run check` Exit 0, Doku-Check „Keine Befunde" |
| Regel-Gate | `scripts/check-rules.mjs` | projektspezifische Regeln, zwei Mechaniken: AST-Regeln über die TypeScript Compiler API (noch leer) und Textregeln über `.ts`/`.mjs`/`.cjs`. Registriert: **(R1)** kein rekursives Löschen ohne `maxRetries` in Prüfcode (Tests, `scripts/check-*`, `verify-*`, `erzeuge-*`) — F-257. Textregel und nicht AST, weil die Gate-Skripte des Harness `.mjs` sind und einer TS-AST-Mechanik gar nicht erst begegnen. | **(R1) real, nicht konstruiert:** der `raeumeVerzeichnis`-Aufruf in `raeumeAuf()` in `src/auftrag/auftrag.test.ts` temporär auf den nackten Vorzustand (`rmSync` mit `recursive`/`force`, ohne `maxRetries`) zurückgebaut, 11.09.2026 → 1 Befund `(R1) src/auftrag/auftrag.test.ts: … ohne maxRetries — raeumeVerzeichnis() aus scripts/_aufraeumen.ts benutzen (F-257)`, Exit 1. **Zusätzlich im Gate selbst, bei jedem Lauf:** drei konstruierte Verstöße (einzeilig, verschachteltes Argument, über drei Zeilen verteilt) müssen erkannt werden; erkennt das Muster einen nicht, meldet das Gate sich selbst als wirkungslos. **Dateiauswahl:** `istPruefcode` muss drei bekannte Prüfdateien erkennen, drei Nicht-Prüfdateien (`src/auftrag/index.ts`, `scripts/leitstand-server.mjs`, `scripts/_aufraeumen.ts`) nicht, und die Sammlung muss sich selbst finden (`scripts/check-rules.mjs` ist Prüfcode nach dem eigenen Muster) — sonst wäre ein falscher Startordner oder eine kaputte Pfadnormalisierung still grün. Bewusst keine feste Untergrenze an Dateien: die bände das Gate an den Bestand dieses Projekts und machte `check:template` im leeren Template rot (F-191). | Zurückgebaute Zeile wiederhergestellt → `node scripts/check-rules.mjs` „✓ (R1)-Kalibrierung … findet 46 Prüfdateien." + „✓ Keine Befunde.", Exit 0; `npm run check` Exit 0, 383/383 Tests. Grün-Gegenprobe im Gate: `…{ recursive: true, force: true, maxRetries: 10, retryDelay: 100 }` und ein nicht-rekursives `{ force: true }` auf eine Einzeldatei erzeugen keinen Befund. |
...
| F5-Context-Builder-Gate | `scripts/check-f5-context-builder.mjs` | `baueKontextpaket`/`pruefeKontextpaketFrisch`/`elementSchluessel`/`passtMuster`/`validiereKontextpaketDaten`, direkt aus `src/context-builder/` importiert (kein zweiter Regelsatz, D5-Muster). Ruft F2s `registriereKernArtefakt`/`pruefeStale` ausschließlich von außen auf (D1, kein Touch). Fünf Payload-Fixtures gegen `validiereKontextpaketDaten`; ein synthetischer Ende-zu-Ende-Lauf (Anfragen → Kontextpaket → Registrierung → `pruefeKontextpaketFrisch` liefert `stale:false`); ein Rollenausschluss-Fall (`code-reviewer` gegen `state/tasks/**`); ein Evidenzlücke-Fall (notwendige Anfrage über Budget → `EVIDENZLUECKE`, kein Teilpaket, Entscheidung 115). Elf `node:test`-Fälle (AC1–AC7, AC10/Delta 5, Delta 2, Nachtrag V3, Nachtrag V4) sind `context-builder.test.ts` zugeordnet, nicht diesem Gate-Skript (Testdatei-Zuordnung analog F1/F2/F3/F9-Muster) | 2026-08-31, Vertrag `f5-context-builder`: Fixture-Rot-Fall — `kontrollzustand-kontextpaket.invalid-fehlende-rolle.json` temporär in die `kontrollzustand-kontextpaket-leer.valid.json`-Position kopiert (Original vorher als `.bak` gesichert), `node scripts/check-f5-context-builder.mjs` gelaufen → Exit 1, `schemas/examples/kontrollzustand-kontextpaket-leer.valid.json: sollte gültig sein, aber verletzt: 'rolle' muss ein nicht-leerer String sein`, danach Original wiederhergestellt (`git status --short schemas/examples/` zeigt keinen `.bak`-Rest) → Exit 0. Zusätzlich drei reale, temporäre Codeeingriffe in `src/context-builder/index.ts`, je gegen `context-builder.test.ts` kalibriert, danach zurückgenommen (`grep -rn "TEMP-ROT-FALL" src/` zeigt danach keinen Treffer): **Rollenfilter invertiert** (`!ausschlussmuster.some(...)`) → 8 Fehlschläge (AC1, AC2, AC3, AC4, AC5, AC6, AC7, Nachtrag V4) — belegt, dass der Rollenfilter vor Duplikat-/Widerspruchs- und Budgetprüfung real greift. **Budget-Phasentrennung aufgehoben** (`notwendige` immer leer, alle Anfragen als `optionale` behandelt) → 1 Fehlschlag, exakt AC5 — belegt real, dass D7 (notwendige Anfragen zuerst, Evidenz vor Budget) trägt. **Rollenprüfung und `#`-Validierung deaktiviert** (`if (false && …)`) → 2 Fehlschläge, exakt Delta 2 und Nachtrag V3 — belegt, dass Fail-Closed bei unbekannter Rolle und die `#`-Sperre real greifen | 2026-08-31, unveränderter Repo-Stand: `node scripts/check-f5-context-builder.mjs` → `✓ 5 Payload-Fixture(s) geprüft.`, `✓ Ende-zu-Ende-Lauf: Anfragen → Kontextpaket → Registrierung → pruefeKontextpaketFrisch liefert stale:false.`, `✓ Rollenausschluss-Fall: state/tasks/**-Anfrage für 'code-reviewer' ausgeschlossen, mit Grund vermerkt.`, `✓ Evidenzlücke-Fall: notwendige Anfrage über Budget führt zu EVIDENZLUECKE, kein Teilpaket (Entscheidung 115).`, `✓ Keine Befunde.`, Exit 0; `node --test src/context-builder/context-builder.test.ts` → `tests 11, pass 11, fail 0`; `npm run check` und `npm run check:template` → je Exit 0 |
| F4-Invocation-Policy-Gate | `scripts/check-f4-invocation-policy.mjs` | Prüft zuerst vier Payload-Fixtures unter `schemas/examples/kontrollzustand-invocation-policy-*.json` (je 1 valid/invalid für Baseline und Wirksamkeitsnachweis) gegen die additiv aus `src/invocation-policy/index.ts` exportierten `validiereBaselineEintrag`/`validiereWirksamkeitsnachweisEintrag` (F5-Muster, kein zweiter Validator). Danach `pruefeStartbedingung1`/`pruefeStartbedingung2`/`pruefeStartfreigabe`/`pruefeAufrufparameter`/`verweigereStart`, direkt aus `src/invocation-policy/` importiert (kein zweiter Regelsatz, D5-Muster). Ruft F3s additiv exportierte `leseAusCommit`/`gitattributesPinntZeilenenden`/`leiteRepoRelativenPfadAb` und F1Bs `schreibeWirkungsmarke` ausschließlich von außen auf (D1, kein Touch). Legt ein Wegwerf-Git-Repo unter `os.tmpdir()` an (`.gitattributes: * -text` vor dem ersten Commit, F3-Muster), committet eine gültige Baseline; prüft Bedingung 1 Grün-Fall + zwei Rot-Fälle (abweichender Schutzskript-Hash E-183, Pfad außerhalb des Repos); Bedingung 2 Grün-Fall + Drift-Fall (F11-Querkonsistenz — Nachweis mit abweichendem `schutzskript_hashes`-Eintrag gegenüber demselben, bereits in Bedingung 1 verifizierten `istZustand`); `pruefeAufrufparameter` Grün-Fall (leere Liste) + Rot-Fall (`--dangerously-skip-permissions`, E-182); AC8-Grep gegen die Produktionsdateien von `src/invocation-policy/*.ts` (`invocation-policy.test.ts` bewusst ausgenommen — die Testdatei startet nie das geprüfte Werkzeug, nur `git` für ihre eigene Fixture, AC8/feature.md verbietet wörtlich nur den Kindprozess "des zu prüfenden Werkzeugs"). Sechs `node:test`-Fälle (AC10 Fall 1–4, F11-Querkonsistenz, AC7) sind `invocation-policy.test.ts` zugeordnet, nicht diesem Gate-Skript (Testdatei-Zuordnung analog F3/F9/F5-Muster) | 2026-08-31, Vertrag `f4-invocation-policy`: Rot-Fälle im Gate-Skript über echte, manipulierte Fixtures erzeugt (F3-Muster, kein Code-Eingriff in `src/invocation-policy/index.ts` nötig) — `node scripts/check-f4-invocation-policy.mjs` zeigt für jeden Fall die reale Ablehnungsmeldung (siehe Grün-Spalte). Zusätzlich fünf reale, temporäre Codeeingriffe in `src/invocation-policy/index.ts` bzw. `verbotene-aufrufparameter.ts` (`TEMP-ROT-FALL A`–`E`), je gegen `invocation-policy.test.ts` kalibriert, danach zurückgenommen (`grep -rn "TEMP-ROT-FALL" src/` zeigt danach keinen Treffer): **A (Fall 1/FREIGEGEBEN)** — Vergleich `werkzeug_konfiguration_hash` invertiert (`!==` → `===`) → 3 Fehlschläge (Fall 1, Fall 3, F11), Fall 1 verliert `FREIGEGEBEN`, Fall 3 verliert die erwartete `E-188`-Begründung (bekommt stattdessen `E-183`), F11 verliert `bedingung1.ok === true`. **B (Fall 2/E-183)** — `schutzskriptHashSaetzeGleich`-Prüfung in Bedingung 1 mit `if (false && ...)` deaktiviert → genau 1 Fehlschlag (Fall 2), manipuliertes Schutzskript wird fälschlich akzeptiert. **C (Fall 3/E-188)** — `arbeitsverzeichnis_pfad`-Vergleich in Bedingung 2 mit `if (false && ...)` deaktiviert → genau 1 Fehlschlag (Fall 3), Pfad-Drift wird nicht mehr erkannt. **D (Fall 4/E-182)** — `pruefeAufrufparameter`s `includes`-Prüfung mit `if (false && ...)` deaktiviert → genau 1 Fehlschlag (Fall 4), `--dangerously-skip-permissions` wird fälschlich akzeptiert. **E (F11-Querkonsistenz)** — `schutzskript_hashes`-Vergleich in Bedingung 2 mit `if (false && ...)` deaktiviert → genau 1 Fehlschlag (F11-Test), ein Nachweis mit veraltetem `schutzskript_hashes`-Eintrag wird trotz grüner Bedingung 1 fälschlich als `{ok:true}` durchgelassen — belegt real den Fehler, den plan-v2 Delta 1 (Advisor-Finding F11) verhindern sollte. Regressionsbeleg: `node --test src/authorization-boundary/authorization-boundary.test.ts` unverändert `tests 6, pass 6, fail 0` (additiver F3-Export berührt bestehendes Verhalten nicht) | 2026-08-31, unveränderter Repo-Stand: `node scripts/check-f4-invocation-policy.mjs` → `✓ Bedingung 1 Grün-Fall: passender istZustand gegen echte, committete Baseline akzeptiert.`, `✓ Bedingung 1 Rot-Fall (abweichender Schutzskript-Hash): abgelehnt (Schutzskript-Hash(es) weichen von der Baseline ab oder fehlen (E-183)).`, `✓ Bedingung 1 Rot-Fall (Pfad außerhalb): abgelehnt (Baseline-Pfad ausserhalb des erwarteten externen Repos).`, `✓ Bedingung 2 Grün-Fall: Nachweis passt zum Gültigkeitsschlüssel.`, `✓ F11-Querkonsistenz-Fall: Bedingung 1 grün, Bedingung 2 lehnt denselben istZustand mit veraltetem Nachweis-Hash ab (Drift im Gültigkeitsschlüssel: 'schutzskript_hashes' (E-188)) — kein FREIGEGEBEN trotz grüner Bedingung 1.`, `✓ pruefeAufrufparameter Grün-Fall: leere Parameterliste akzeptiert.`, `✓ pruefeAufrufparameter Rot-Fall: '--dangerously-skip-permissions' abgelehnt (verbotener Aufrufparameter (E-182): '--dangerously-skip-permissions').`, `✓ AC8: kein child_process/spawn/exec/execSync in den Produktionsdateien von src/invocation-policy/*.ts (Testfixture ausgenommen, siehe Kommentar) — F4 startet nie selbst einen Werkzeugprozess.`, `✓ Keine Befunde.`, Exit 0; `node --test src/invocation-policy/invocation-policy.test.ts` → `tests 6, pass 6, fail 0`; `npm run check` und `npm run check:template` → je Exit 0, `tests 57, pass 57, fail 0`. 2026-08-31 Nachtrag (Advisor-Befund vor Commit): die vier Payload-Fixtures waren bis dahin von keinem Gate gelesen. `validiereBaselineEintrag`/`validiereWirksamkeitsnachweisEintrag` additiv aus `src/invocation-policy/index.ts` exportiert (bislang modulintern), Gate-Skript um Abschnitt (a) nach F5-Muster ergänzt. Erster Lauf deckte einen echten Fixture-Fehler auf: `kontrollzustand-invocation-policy-baseline.valid.json` und `...wirksamkeitsnachweis.valid.json` enthielten einen auf 62 statt 64 Hex-Zeichen verkürzten `schutzskripte[1].hash`/`schutzskript_hashes[1]`-Wert (`✗ sollte gültig sein, aber verletzt: '...hash' muss ein 64-stelliger Hex-String sein`) — beide Fixtures korrigiert (Hash programmatisch auf exakt 64 `c`-Zeichen gesetzt, Länge per `node -e` verifiziert statt visuell gezählt). Danach `node scripts/check-f4-invocation-policy.mjs` → `✓ 4 Payload-Fixture(s) geprüft.`, `✓ Keine Befunde.`, Exit 0; `npm run check` und `npm run check:template` → je Exit 0, `tests 57, pass 57, fail 0` (unverändert gegenüber vorherigem Stand) |
| F7-Result-Evaluator-Gate | `scripts/check-f7-result-evaluator.mjs` | `klassifiziereLauf`, direkt aus `src/result-evaluator/` importiert (kein zweiter Regelsatz, D5-Muster). Ruft F6as `leseErgebnisobjekt` (additiv exportiert, F-062) und F1Bs `schreibeWirkungsmarke`/`stelleLaufstatusFest` ausschließlich von außen auf (D1, kein Touch). (a) AK4-Grep — kein `.includes(`/`.match(`/`.indexOf(`/`.search(`/RegExp-`.test(` in `src/result-evaluator/*.ts` (ohne `*.test.ts`), Selbsttest mit fünf verschiedenen simulierten Verstoßformen (F-060-Lehre); `permission_denials`-Lesen ausdrücklich erlaubt (anders als F6as AK12) — es ist F7s Aufgabe. (b) Fixture-Prüfung: die realen TP-03d-Messfälle 1-3 und TP-01e Messfall A (F6as `attrappeMitValidemErgebnis`/`attrappeOhneErgebnisobjekt` wörtlich wiederverwendet, D5) laufen durch `klassifiziereLauf` und liefern das erwartete `ergebnis`. (c) Ende-zu-Ende-Beleg: `RUN_PREPARED` → `klassifiziereLauf` schreibt Terminal → `stelleLaufstatusFest` liefert `ABGESCHLOSSEN` mit demselben `ergebnis` (AK6). 17 `node:test`-Fälle (drei Terminalausgänge, Hash-Abweichung, fehlende Rohstrom-Datei, zwei `kein_ergebnisobjekt`-Diagnosefälle, AK2-Priorität, fehlendes/nicht-Array-`permission_denials`-Feld, zwei E-186-Zählfälle inkl. Shell-Quoting-Einbettung, ein dokumentierter Adapter-Grenzfall ohne Wortgrenze, `is_error`/`non_execution_kind` informativ in beiden Zweigen, TP-01e Messfall A, ein dokumentierter F1B-Doppelaufruf-Fall) sind `result-evaluator.test.ts` zugeordnet, nicht diesem Gate-Skript (Testdatei-Zuordnung analog F1/F2/F3/F9/F5/F4-Muster) | 2026-09-02, Vertrag `f7-result-evaluator`: realer, temporärer Codeeingriff in `src/result-evaluator/index.ts` (`tempRotfallAk4`, `stdout.includes(...)`, `TEMP-ROT-FALL`-Kommentar) → `node scripts/check-f7-result-evaluator.mjs` meldet genau einen Befund, `AK4: verbotenes Muster (Konsolentext-Ableitung) in src/result-evaluator/index.ts gefunden`, Exit 1; danach zurückgenommen (`grep -rn "TEMP-ROT-FALL" src/result-evaluator/` zeigt danach keinen Treffer). Der AK4-Selbsttest selbst deckt fünf unterschiedliche Verstoßformen ab (`.includes(`, `.match(`, `.indexOf(`, `.search(`, RegExp-`.test(`) — kein Einzelfall wie bei F-060 | 2026-09-02, unveränderter Repo-Stand: `node scripts/check-f7-result-evaluator.mjs` → `✓ AK4: kein Codepfad …`, `✓ AK4-Selbsttest: alle 5 simulierten Verstoßformen … werden vom Muster erkannt.`, vier Fixture-Zeilen (`✓ Fixture 'TP-03d Messfall 1 …': ERFOLGREICH wie erwartet.` usw.), `✓ Ende-zu-Ende-Lauf: RUN_PREPARED → klassifiziereLauf schreibt Terminal über F1B → stelleLaufstatusFest liefert ABGESCHLOSSEN/ERFOLGREICH (AK6).`, `✓ Keine Befunde.`, Exit 0; `node --test src/result-evaluator/result-evaluator.test.ts` → `tests 17, pass 17, fail 0`; `npm run check` und `npm run check:template` → je Exit 0, `tests 87, pass 87, fail 0`. **Nachtrag 02.09.2026 (Reviewer-/QA-Pass, frischer Kontext, F-046):** beide Pässe „Freigegeben mit Hinweisen", keine Blocker. Vor Commit nachgezogen: sechs zusätzliche Testfälle (zwei `kein_ergebnisobjekt`-Diagnosefälle, `permission_denials`-Nicht-Array-Wert, `is_error`/`non_execution_kind` im VERWEIGERT-Zweig, ein dokumentierter Adapter-Grenzfall ohne Wortgrenze, ein dokumentierter F1B-Doppelaufruf-Fall), ein Typsicherheits-Fix (`istPermissionDenial`-Typ-Guard statt ungeprüftem `as`-Cast in `index.ts`), AK4-Selbsttest um `.search(` ergänzt, doppelte Zeile „10." in `feature.md` Nachweis bereinigt. Vier neue Findings (F-064 bis F-067) für die verbleibenden, bewusst nicht in diesem Baudurchgang behobenen Reviewer-/QA-Punkte. |
| F8-Execution-Controller-Gate (WS-1/WS-2a/WS-2b) | `scripts/check-f8-execution-controller.mjs` | `fuehreAufgabeDurch`, direkt aus `src/execution-controller/` importiert. Ruft F5s `baueKontextpaket`, F6as `baueAufruf`/`starteGateway`, F7s `klassifiziereLauf` und F1Bs `stelleLaufstatusFest` ausschließlich von außen auf (D1, kein Touch). (a) AK1-Grep — kein Vorkommen von `ROLLEN_AUSSCHLUSSMUSTER`/`pruefeUndVerweigereBeiTreffer`/`ermittleErgebnis`/`permission_denials`/`non_execution_kind` in den Produktionsdateien von `src/execution-controller/*.ts` (Testdatei ausgenommen, Vertrag-SCOPE Punkt 3.1), Selbsttest mit einem simulierten `permission_denials`-Verstoß. (b) AK3-Grep — kein Vorkommen von `pruefeStartfreigabe`/`ermittleIstZustand`/`aktuelle-autorisierung` in den Produktionsdateien von `src/execution-controller/*.ts`, **seit Entscheidung Stefan 04.09.2026 mit derselben Testdatei-Ausnahme wie AK1** (hebt die ursprünglich im Vertrag angelegte Asymmetrie zu AK1 auf — die ursprüngliche Zeile unten bleibt stehen und ist mit dem Nachtrag als überholt gekennzeichnet, siehe `features/F8/journal.md`) — Selbsttest mit einem simulierten `ermittleIstZustand`-Aufruf. Drei `node:test`-Fälle (AK1/AK5/AK8 Grün-Durchlauf, AK2/AK8 F6a-Rot-Fall, F5-Gegenstück-Rot-Fall `stufe:'kontextpaket'`) sind `execution-controller.test.ts` zugeordnet, nicht diesem Gate-Skript (Testdatei-Zuordnung analog F1/F2/F3/F9/F5/F4/F7-Muster). „Je genau einmal" (AK1) hat hier keine injizierbare Spy-Stelle wie F6as `starter` — real belegt über F2-`versionSequenz` (bliebe nicht bei 1, wenn `baueKontextpaket`/`starteGateway` zweimal liefen) und die FIFO-Paarung der Wirkungsmarken-Kette (`terminaleOhneRunPrepared` bleibt leer, was bei einem zweiten `klassifiziereLauf`-Aufruf nicht der Fall wäre). AK2s Nichtaufruf-Nachweis (F-103, seit 04.09.2026): nicht "kein terminal-Eintrag" absolut — F6as eigenes `verweigereStart` schreibt bei diesem Rot-Fall bereits selbst einen (sequenz 1) —, sondern "kein zweiter" (Kettenlänge bleibt 1 statt 2 bei einem zusätzlichen `klassifiziereLauf`-Aufruf) **Nachtrag 04.09.2026 (Vertrag `f8-execution-controller-ws2a`):** AK4/AK6 und der Delta-1-Wurfpfad ergänzt (WS-2a, E-186-Eskalation über F9 zwischen Schritt 4 und Schritt 5, plan-v2 Delta 2). Die Gate-Datei selbst bleibt unverändert (Vertrag-SCOPE Punkt 7, AK1-/AK3-Grep-Listen nicht um `bypass_verdacht_anzahl`/`erfasseBedarf`/`haendigeAus` erweitert), fünf neue `node:test`-Fälle in `execution-controller.test.ts`: `Attrappen-Vorprüfung` (belegt, dass die beiden neuen `VERWEIGERT`-Starter-Attrappen real `bypass_verdacht_anzahl` 1 bzw. 0 erzeugen, F-103-Muster), `AK4-positiv`, `AK4-negativ`, `AK6 (F-091)`, `Delta 1 (Wurf)` (Wurf in `erzeugeTransportpaket` über einen realen Vorbedingungsbruch ausgelöst — ein `schreiber`-Hook löscht das gerade geschriebene BEDARF_V0-Lineage-Verzeichnis, kein Modul-Mock, D1) | **[ÜBERHOLT seit 04.09.2026, siehe Nachtrag unten]** 2026-09-04, Vertrag `f8-execution-controller-ws1`: zwei separate, reale Codeeingriffe (`TEMP-ROT-FALL-KALIBRIERUNG`-Kommentar), je einzeln eingefügt, geprüft, zurückgenommen. AK1: Kommentarzeile `// TEMP-ROT-FALL-KALIBRIERUNG: permission_denials` in `src/execution-controller/index.ts` → `node scripts/check-f8-execution-controller.mjs` → Exit 1, `AK1: verbotenes Muster (F5/F6a/F7-Regelbezeichner) in src/execution-controller/index.ts gefunden`. AK3 (damals ohne Testdatei-Ausnahme): Kommentarzeile `// TEMP-ROT-FALL-KALIBRIERUNG: ermittleIstZustand` in `src/execution-controller/execution-controller.test.ts` (bewusst in der Testdatei, um die damals fehlende Testausnahme real zu belegen) → Exit 1, `AK3: verbotenes Muster (F4-Startfreigabebezeichner) in src/execution-controller/execution-controller.test.ts gefunden`. Beide Zeilen danach entfernt (`grep -rn "TEMP-ROT-FALL" src/execution-controller/` zeigt danach keinen Treffer) **Nachtrag 04.09.2026, Vertrag `f8-execution-controller-ws2a`:** AK4-negativ-Kalibrierung (SCOPE Punkt 5, Rot-Fall-Kalibrierungslauf) — Eskalationsbedingung in `src/execution-controller/index.ts` testweise von `bypass_verdacht_anzahl > 0` auf `>= 0` gesetzt → `node --test src/execution-controller/execution-controller.test.ts` → `tests 8, pass 7, fail 1`, `AK4-negativ` rot: `AssertionError [ERR_ASSERTION]: keine Eskalation bei bypass_verdacht_anzahl === 0`, `actual: { laufId: 'ak4n-...-eskalation-...', bedarfVersionSequenz: 1, transportVersionSequenz: 1 }` gegen `expected: undefined` — belegt, dass der Nichtaufruf-Nachweis (F-103) real an einer Stelle hängt, die im Positivfall nachweislich anders aussieht; danach zurückgebaut | **[ÜBERHOLT seit 04.09.2026, siehe Nachtrag unten]** 2026-09-04, unveränderter Repo-Stand: `node scripts/check-f8-execution-controller.mjs` → `✓ AK1: kein Vorkommen …`, `✓ AK1-Selbsttest: simulierter Verstoß wird vom Muster erkannt.`, `✓ AK3: kein Vorkommen … (inkl. Testdateien)`, `✓ AK3-Selbsttest: simulierter Verstoß wird vom Muster erkannt.`, `✓ Keine Befunde.`, Exit 0; `node --test src/execution-controller/execution-controller.test.ts` → `tests 2, pass 2, fail 0`; `npm run check` und `npm run check:template` → je Exit 0, `tests 118, pass 118, fail 0`. Optionen-Form-Frage (plan-v1 Abschnitt 10 Frage 4, plan-v2 Delta 3) hier gelöst: `AusfuehrungsOptionen` mit einem einzigen, nullstelligen `schreiber?: () => void` (Muster F6a `GatewayOptionen.schreiber`) plus einem gemeinsamen `basisVerzeichnis?` und den vier F6a-spezifischen Feldern (`rohBasisVerzeichnis`, `starter`, `settingsPfad`, `aktuelleAutorisierungPfad`, `startfreigabeRepoWurzel`), unverändert an F5/F6a/F7/F1B durchgereicht, keines vom Controller selbst gelesen. **Nachtrag 04.09.2026 (Korrekturrunde, Entscheidung Stefan: AK3-Grep erhält dieselbe `*.test.ts`-Ausnahme wie AK1, F-103/F-104, siehe `features/F8/journal.md`):** AK3-Rot-Fall neu kalibriert gegen den neuen Zuschnitt (Produktionsdateien) — Kommentarzeile `// TEMP-ROT-FALL-KALIBRIERUNG: ermittleIstZustand` in `src/execution-controller/index.ts` (jetzt Produktionsdatei statt Testdatei) → `node scripts/check-f8-execution-controller.mjs` → Exit 1, `AK3: verbotenes Muster (F4-Startfreigabebezeichner) in src/execution-controller/index.ts gefunden`; danach entfernt. Grün-Fall neu belegt: dasselbe Vorkommen jetzt ausschließlich in `execution-controller.test.ts` (real, über den direkten `ermittleIstZustand`-Aufruf der F4-Fixture, nicht mehr simuliert) → `node scripts/check-f8-execution-controller.mjs` → Exit 0, `✓ AK3: kein Vorkommen … in den Produktionsdateien von src/execution-controller/*.ts.` — belegt, dass ein Testdatei-Vorkommen jetzt NICHT mehr anschlägt (Zweck der Änderung). Zusätzlich AK2-Nichtaufruf-Nachweis (F-103) real kalibriert: Early-Return `execution-controller/index.ts:77-79` temporär deaktiviert (`if (false && !gatewayErgebnis.ok)`, `TEMP-ROT-FALL-KALIBRIERUNG`-Kommentar) → `klassifiziereLauf` lief real im Rot-Fall, terminal-Kette wuchs von 1 auf 2 Einträge (real per Probe-Skript und `node --test` bestätigt: `AssertionError [ERR_ASSERTION]: Expected values to be strictly equal: true !== false` an der `ergebnis.ok`-Assertion, die vor der neuen Kettenlängen-Assertion greift — Testlauf insgesamt rot, `tests 3, pass 2, fail 1`); danach zurückgebaut, `grep -rn "TEMP-ROT-FALL" src/execution-controller/` zeigt keinen Treffer, `node --test src/execution-controller/execution-controller.test.ts` wieder `tests 3, pass 3, fail 0`. `npm run check` und `npm run check:template` → je Exit 0, `tests 119, pass 119, fail 0` (118 + 1 neuer Testfall F5-Gegenstück-Rot-Fall). **Nachtrag 04.09.2026, Vertrag `f8-execution-controller-ws2a`:** unveränderter Repo-Stand (Eskalationsbedingung wieder `> 0`) → `node --test src/execution-controller/execution-controller.test.ts` → `tests 8, pass 8, fail 0` (die 3 WS-1- plus 5 neue WS-2a-Testfälle: `Attrappen-Vorprüfung`, `AK4-positiv`, `AK4-negativ`, `AK6 (F-091)`, `Delta 1 (Wurf)`); `npm run check` und `npm run check:template` → je Exit 0, `tests 124, pass 124, fail 0`. Längster real erzeugter Pfad unter den WS-2a-Testfällen (F-109, laufId-Präfixe ≤4 Zeichen) real gemessen: 249 Zeichen ab Repo-Wurzel absolut (`...\kontrollzustand-test\lineage-transport-ak4n-<uuid>-eskalation-<uuid>\checkpoints\1-<64-Hex>.json`) — unter der Windows-MAX_PATH-Grenze von 260. **Nachtrag 04.09.2026, Vertrag `f8-execution-controller-ws2b`:** AK7 gebaut — `AusfuehrungsEingaben` um optionales `vorgaengerLaufId` erweitert, `fuehreAufgabeDurch` lädt bei gesetztem Feld die Laufakte des Vorgängerlaufs über `ladeArtefaktVersion` und stellt sie als `notwendig:true`-Anfrage voran; fehlt die Laufakte, wirft die Funktion (Vorbedingungsverletzung). Die Gate-Datei selbst bleibt unverändert (Vertrag-SCOPE Punkt 4, Design-Entscheidung: ein Grep gegen die mehrzeilige `starteGateway`-Aufrufstelle wäre nicht robust baubar; der Vorher/Nachher-Test belegt die Isolation direkt und strenger). Drei neue `node:test`-Fälle in `execution-controller.test.ts`: `AK7-positiv-A (KLAERUNG_ERFORDERLICH)` (Vorgängerlauf über einen direkten `starteGateway`-Aufruf ohne `klassifiziereLauf` erzeugt, Lineage-Verweis gegen die real geladene Vorgänger-Laufakte geprüft, Isolationsnachweis über echten Vorher/Nachher-Vergleich von `stelleLaufstatusFest(vorgaengerLaufId)`), `AK7-positiv-B (FEHLGESCHLAGEN)` (Vorgängerlauf real über `fuehreAufgabeDurch` mit `attrappeOhneErgebnisobjekt` bis `FEHLGESCHLAGEN` durchlaufen, dasselbe Muster), `Vorbedingungsverstoß` (`vorgaengerLaufId` ohne Laufakte → `assert.rejects`, Fehlertext enthält die `vorgaengerLaufId`). Beleg: `node scripts/check-f8-execution-controller.mjs` → unverändert `✓ AK1: kein Vorkommen …`, `✓ AK3: kein Vorkommen …`, beide Selbsttests grün, `✓ Keine Befunde.`, Exit 0; `npm run check:template` und `npm run check` → je Exit 0, `npm run check` `tests 127, pass 127, fail 0` (124 + 3 neue WS-2b-Testfälle). |
| F10-Leitstand-Gate | `scripts/check-f10-leitstand.mjs` | AK1 (reales, valides Profil unter `profiles/`), AK3 (Options-Sperre gegen `AusfuehrungsOptionen`-Felder und unbekannte Body-Felder im Startauftrag), AK4 (Loopback-Bindung `127.0.0.1`), AK5 (laufId-Eindeutigkeit, synchrone Reservierung vor `fuehreAufgabeDurch`), AK6 (Pflicht-`.catch()` um den Fire-and-forget-Aufruf, flüchtige `GET /api/startfehler`-Projektion); seit F11 WS-2 zusätzlich real gegen einen Testserver: F11 AK5 (Startvorlage-Felder/freie Werkzeugliste im Body → 400), F11 AK6 (`inhalt` im Body, unsichere Pfade, fehlende Evidenzdatei → je 400), F11 AK7/D13 (zweiter Start mit ANDERER laufId während laufendem Lauf → 409) | 2026-09-06, F11 WS-2 real kalibriert: (1) AK5 — `VERBOTENE_STARTVORLAGE_FELDER` in `scripts/leitstand-server.mjs` testweise auf ein leeres Set gesetzt UND `profilReferenz`/`werkzeugStartziel`/`werkzeugVersionDeklariert`/`berechtigungskontext` testweise zu `ERLAUBTE_STARTAUFTRAG_FELDER` hinzugefügt → `node scripts/check-f11-auftrag.mjs` → Exit 1, `AK5-Rotfall: Body mit 'profilReferenz' sollte abgelehnt werden, wurde durchgelassen`; Original wiederhergestellt → Exit 0. (2) AK6 — `isAbsolute(pfad)`-Prüfung in `loeseEvidenzPfadAuf` testweise mit `false &&` deaktiviert → Exit 1, `AK6-Rotfall: folgende Pfade sollten abgelehnt werden, wurden aber durchgelassen: C:\Windows\win.ini, /etc/passwd` (die beiden `..`-Pfade blieben dagegen weiterhin abgelehnt — zweite Verteidigungsschicht über `leiteRepoRelativenPfadAb` greift unabhängig); Original wiederhergestellt → Exit 0. (3) AK7/D13 — `laufAktiv = false` im `.catch`-Zweig von `erzeugeRequestHandler` testweise entfernt → Exit 1, `AK7: scripts\leitstand-server.mjs erfüllt den D13-Vertrag nicht erkennbar`; Original wiederhergestellt → Exit 0. Reale Live-Server-Rotfälle direkt gegen `scripts/check-f10-leitstand.mjs` (unveränderter Code, echte Anfragen): Body mit `profilReferenz` bzw. `werkzeugStartziel` → 400; Anfrage mit `inhalt` → 400; Anfrage mit `pfad: '../ausserhalb.txt'`/`'C:\\Windows\\win.ini'`/`'/etc/passwd'` → je 400; Anfrage mit nicht existierender Datei → 400; zweiter Start mit anderer laufId unmittelbar nach einem noch laufenden Lauf → 409 mit `'D13'` im Grund | 2026-09-06, unveränderter Repo-Stand nach jeder Rückstellung oben: `npm run check` → Exit 0, `tests 143, pass 143, fail 0`; `node scripts/check-f10-leitstand.mjs` → u. a. `✓ AK1: profiles/beispielprojekt.json valide.`, `✓ F11 AK5: alle 4 Startvorlage-Felder plus eine freie 'aufrufEingaben.werkzeugsatz'-Liste werden mit 400 abgelehnt.`, `✓ F11 AK6: verbotenes 'inhalt', drei unsichere Pfade und eine fehlende Datei werden je mit 400 abgelehnt.`, `✓ F11 AK7 (D13): ein zweiter Start mit ANDERER laufId während eines laufenden Laufs bekommt 409 ('D13' im Grund), nach dessen Rückkehr gelingt ein neuer Start.`, `✓ Keine Befunde.`, Exit 0 |
| F11-Auftrag-Gate (WS-1) | `scripts/check-f11-auftrag.mjs` | `validiereAuftragDaten`, direkt aus `src/auftrag/index.ts` importiert (kein zweiter Regelsatz, D5-Muster): (a) drei Payload-Fixtures unter `schemas/examples/kontrollzustand-auftrag*.json`; (b) AK3-Grep + Selbsttest — kein Vorkommen von `auftragstext` im anfragen-Konstruktionsblock von `src/execution-controller/index.ts` (zwischen Funktionsstart und dem `baueKontextpaket`-Aufruf), nur im nachfolgenden Prompt-Zusammensetzungsteil. AK1 (`registriereAuftrag`-Unit-Test) und AK2/AK3(b) (Prompt-Trennung `===`, Laufzeit-Marke-Nachweis) sind `src/auftrag/auftrag.test.ts` bzw. `execution-controller.test.ts` zugeordnet, nicht diesem Gate-Skript (Testdatei-Zuordnung analog F1/F2/F3/F9/F5/F4/F7/F8-Muster) | 2026-09-06, Vertrag `f11-auftrag-ws1`: (1) Fixture-Rot-Fall — `kontrollzustand-auftrag.invalid-leerer-auftragstext.json` temporär in die `kontrollzustand-auftrag.valid.json`-Position kopiert (Original als `.bak` gesichert) → Exit 1, `schemas/examples/kontrollzustand-auftrag.valid.json: sollte gültig sein, aber verletzt: 'auftragstext' muss ein nicht-leerer String sein`; Original wiederhergestellt (`git status --short schemas/examples/` zeigt danach keinen `.bak`-Rest) → Exit 0. (2) AK3(a)-Rot-Fall — reale, temporäre Zeile `const TEMP_ROT_FALL = eingaben.auftragstext` unmittelbar vor der `vorgaengerLaufId`-Prüfung in `src/execution-controller/index.ts` (also im anfragen-Konstruktionsblock) eingefügt → `node scripts/check-f11-auftrag.mjs` → Exit 1, `AK3: 'auftragstext' kommt … im anfragen-Konstruktionsblock … vor — würde in die Anfragenliste durchsickern`; danach entfernt (`grep -rn "TEMP_ROT_FALL" src/` zeigt danach keinen Treffer) → Exit 0. (3) AK3(b)-Rot-Fall (Laufzeit-Test statt Gate-Skript, gegen `execution-controller.test.ts` kalibriert) — reale, temporäre Zeile `anfragen = [...anfragen, { pfad: eingaben.auftragstext, frage: 'x', begruendung: 'x', inhalt: 'x' }]` unmittelbar vor dem `baueKontextpaket`-Aufruf → `node --test src/execution-controller/execution-controller.test.ts` → `tests 15, pass 12, fail 3` (F11 AK3(b) real rot: `AssertionError … Auftragstext-Marke darf nicht im registrierten Kontextpaket auftauchen, true !== false`; die beiden F11-AK2-Tests fallen als Kollateraleffekt derselben Injektion ebenfalls rot, da sie denselben Evidenztext prüfen); danach entfernt, `node --test src/execution-controller/execution-controller.test.ts` wieder `tests 15, pass 15, fail 0`. (4) F11-AK2-Rot-Fall in `check-f10-leitstand.mjs` — `'auftragstext'` testweise aus `PFLICHT_STARTAUFTRAG_FELDER` in `scripts/leitstand-server.mjs` entfernt (Original als `.bak` gesichert) → `node scripts/check-f10-leitstand.mjs` → Exit 1, `F11 AK2: Startauftrag ohne 'auftragstext' erwartet 400 …, erhalten status=202`; Original wiederhergestellt → Exit 0 | 2026-09-06, unveränderter Repo-Stand: `node scripts/check-f11-auftrag.mjs` → `✓ 3 Payload-Fixture(s) geprüft.`, `✓ AK3: kein Vorkommen von 'auftragstext' im anfragen-Konstruktionsblock …`, `✓ AK3-Selbsttest: simulierter Verstoß … wird erkannt.`, `✓ Keine Befunde.`, Exit 0; `node --test src/auftrag/auftrag.test.ts` → `tests 1, pass 1, fail 0`; `node --test src/execution-controller/execution-controller.test.ts` → `tests 15, pass 15, fail 0`; `node scripts/check-f10-leitstand.mjs` → `✓ F11 AK2: Startauftrag ohne 'auftragstext' wird mit 400 abgelehnt …`, `✓ Keine Befunde.`, Exit 0; `npm run check` und `npm run check:template` → je Exit 0, `npm run check` `tests 137, pass 137, fail 0` (133 + 4 neue Testfälle: 1 in `auftrag.test.ts`, 3 F11-Fälle in `execution-controller.test.ts`; die 133 selbst liegen über der zuletzt in dieser Tabelle dokumentierten Zahl 127, weil zwischen dem F8-WS2b-Vertrag und diesem hier F10 WS-1/WS-2 real gebaut wurden, siehe `features/F10/journal.md`). Zusätzlich F11 WS-2 (2026-09-06, AK4/AK5/AK6/AK7/AK9): `scripts/check-f11-auftrag.mjs` um vier mechanische Blöcke erweitert, alle direkt gegen die reinen, aus `scripts/leitstand-server.mjs` exportierten Funktionen (`pruefeStartauftrag`, `loeseEvidenzPfadAuf`) bzw. per Grep+Selbsttest gegen dessen Quelltext (AK7/D13, closure-gekapselter Zustand, ohne Live-Server nicht direkt aufrufbar) — Rot-/Grün-Fall-Belege siehe die neu gefasste F10-Leitstand-Gate-Zeile oben (dieselben drei realen Kalibrierläufe deckten sowohl das F10- als auch dieses F11-Gate ab, da AK5/AK6/AK7 in beiden geprüft werden). `AK7`-Selbsttest deckte dabei einen realen Kalibrierungsfehler des eigenen Gates auf: eine erste Rotfall-Injektion (`.catch`-Reset per Kommentar "entfernt", Kommentartext enthielt aber wörtlich "laufAktiv = false") wurde vom reinen Text-/Regex-Vergleich fälschlich als vorhanden erkannt (falsches Grün) — nach Umformulierung der Injektion (Kommentar ohne den Literalstring) real als Exit 1 reproduziert; als bekannte Grenze im Gate-Skript selbst dokumentiert. `npm run check` → Exit 0, `tests 143, pass 143, fail 0`. |
| F12-WS-3-Leitstand-Ansicht-Gate | `scripts/check-f12-leitstand-ansicht.mjs` | AK1 (reine Artefaktkette erscheint nicht in `GET /api/laeufe`, Regressionsschutz), AK2 (`GET /api/laeufe` enthält kein `checkpoints`-Array je Eintrag, Regressionsschutz), AK5 (Body mit `auftragstext` → 400, unbekannte `auftragId` → 400 — isolierter Beleg zusätzlich zu den WS-2-Fällen in `check-f10-leitstand.mjs`/`check-f11-auftrag.mjs`), AK8 (gültiger Rohstrom → volle Projektion; manipulierter Rohstrom, Hash weicht ab → AUSSCHLIESSLICH `{status:'hash_weicht_ab'}`, kein `exitCode`/`permissionDenials`/sonstiges Feld) | 2026-09-07, Auftrag F12-WS-3: vier separate, reale Codeeingriffe in `scripts/leitstand-server.mjs` (Original vorher als `.bak` gesichert, jeder Eingriff einzeln zurückgestellt vor dem nächsten). (1) AK1 — `istLaufkette` testweise auf `return true` gesetzt → `node scripts/check-f12-leitstand-ansicht.mjs` → Exit 1, `AK1: reine Artefaktkette 'lineage-auftrag-<id>' … erscheint fälschlich in GET /api/laeufe`; Original wiederhergestellt → Exit 0. (2) AK2 — `sammleLaufKopfdaten`s Rückgabeobjekt testweise um `checkpoints: []` ergänzt → Exit 1, `AK2: GET /api/laeufe-Eintrag für '<laufId>' … enthält noch 'checkpoints'`; Original wiederhergestellt → Exit 0. (3) AK5 — `auftragstext` testweise aus `VERBOTENE_AUFTRAG_FELDER` UND zusätzlich in `ERLAUBTE_STARTAUFTRAG_FELDER` aufgenommen (ein alleiniges Entfernen aus `VERBOTENE_AUFTRAG_FELDER` griff nicht — das Feld fiel weiterhin über die generische "unbekanntes Feld"-Ablehnung durch, echte Tiefenverteidigung real bestätigt) → Exit 1, `AK5: Startauftrag mit 'auftragstext' im Body erwartet 400, erhalten 202`; Original wiederhergestellt → Exit 0. (4) AK8 — die Hash-Vergleichsbedingung in `baueRohstromProjektion` testweise auf `if (false)` gesetzt (D2-Reihenfolge deaktiviert) → Exit 1, `AK8-Rotfall: manipulierter Rohstrom erwartet AUSSCHLIESSLICH {status:'hash_weicht_ab'} …, erhalten {"status":"ok","exitCode":1,…}`; Original wiederhergestellt → Exit 0. Alle vier Rückstellungen über `git diff --stat scripts/leitstand-server.mjs` gegen den Ausgangsstand geprüft (keine Restdifferenz) | 2026-09-07, unveränderter Repo-Stand: `node scripts/check-f12-leitstand-ansicht.mjs` → `✓ AK1 …`, `✓ AK2 …`, `✓ AK5 …`, `✓ AK8: gültiger Rohstrom liefert die volle Projektion; ein manipulierter Rohstrom (Hash weicht ab) liefert AUSSCHLIESSLICH { status: 'hash_weicht_ab' } — kein stiller Durchlauf.`, `✓ Keine Befunde.`, Exit 0; `npm run check` und `npm run check:template` → je Exit 0, `tests 146, pass 146, fail 0`. Realer K1-Beleg gegen den echten `kontrollzustand/` (`e2e-referenzfeature-2026-09-06`, `kontrollzustand-roh/` auf dieser Maschine noch vorhanden): `GET /api/laeufe/e2e-referenzfeature-2026-09-06` → `auftrag:{"status":"kein_auftragsbezug"}`, `laufakte:{"status":"ok",…,"beobachtungsbasisVollstaendig":false}`, `rohstrom:{"status":"ok","exitCode":1,…,"ergebnisobjekt":{"status":"kein_ergebnisobjekt"}}` — deckungsgleich mit der erwarteten Ausprägung, kein `permissionDenials.anzahl:0` trotz unbekannter Beobachtungsbasis. Umgebungs-Nebenbefund während der Kalibrierung: `process.exit()` löste nach mehreren Server-open/close-Zyklen auf diesem Windows-Rechner reproduzierbar einen libuv-Assertionsabsturz aus (`UV_HANDLE_CLOSING`, `src/win/async.c`) — im Gate-Skript durch `process.exitCode` statt `process.exit()` behoben, über 5 aufeinanderfolgende Läufe nach dem Fix bestätigt (Exit 0 in jedem Lauf) |
| F42-Projekt-Harness-Gate (WS-1) — Tabellenpflege ab hier bewusst knapper (spätere Features F13-F41 dokumentieren Rot-/Grün-Fälle primär in ihrer eigenen `features/<id>/feature.md`; dieser Eintrag folgt der AK6-Vorgabe des Auftrags, hält sich aber kurz statt die Tabelle im alten Umfang fortzuschreiben) | `scripts/check-f42-projekt-harness.mjs` | (a) Skelett vorhanden: `vorlagen/projekt-skelett/CLAUDE.md`+`HERKUNFT.md` real in diesem Repo, SHA `9189959` genannt. (b) Baseline unverändert: `kopiereSkelett` überschreibt nie eine bereits vorhandene Datei. (c) `pruefbefehl` gesetzt UND real lauffähig (`fuehrePruefungDurch` gegen ein triviales `check:template`-Script). (d) Coach-Text: `istAiWorkforce:false` nennt weder `validiereRoadmapDaten` noch `check-feature.mjs`, `true` weiterhin beide; UND (F-703-Nachtrag, 25.09.2026) die Prüfbefehl-Zeile zeigt für ECHTE argv (aus `startvorlagen/ai-workforce.json` bzw. real von `schreibeStartvorlageUndProfil` erzeugt) `npm run check`/`npm run check:template` über die neue `anzeigePruefbefehl`-Funktion, nicht das rohe argv; ein Fremd-argv mit Leerzeichen im Pfad wird gequotet. (e) Trust-Erkennung: `pruefeWorkspaceTrust` gegen vier `~/.claude.json`-Fixtures | (a) `kopiereSkelett` gegen eine `installWurzel` OHNE `vorlagen/projekt-skelett/` → wirft real (ENOENT). (b) künstliche Kollision (Fixture-Skelett trägt `.claude/settings.json`) → bereits vorhandener Baseline-Inhalt bleibt unverändert, nicht in der Kopiert-Liste. (c) die vorher geplante, fehlerhafte Form `['npm','run','check:template']` (Advisor-Finding F1, `state/advisor-findings-f42-projekt-harness-ws1.md`) liefert real `ergebnis:'FEHLER'`, nicht `GRUEN` — real reproduziert bei jedem Gate-Lauf. (d) `istAiWorkforce:false` mit `validiereRoadmapDaten` im Text wäre ein Befund (Regressionsschutz); (F-703-Nachtrag) explizit real reproduziert: das vorher tatsächlich ausgelieferte rohe `.join(' ')` der echten `schreibeStartvorlageUndProfil`-argv (`node.exe`- und `npm-cli.js`-Pfad wörtlich im Auftragstext) wird als Befund erkannt (`fremd.auftragstext.includes(echtesNeuesProjektArgv[0])`) — der Gate-Lauf belegt damit denselben Bug, den F-703 beschreibt, nicht nur eine hypothetische Form. (e) je eine Fixture mit falschem Ergebnis pro Status (`true`/`false`/fehlender Schlüssel/fehlende Datei) wäre ein Befund; zusätzlich ein Fixture mit abweichender Laufwerksbuchstabe-Schreibweise muss `'fehlend'` liefern (bewusste Grenze, F-702) | 25.09.2026, unveränderter Repo-Stand: `node scripts/check-f42-projekt-harness.mjs` → alle fünf Prüfungen `✓`, `✓ Keine Befunde.`, Exit 0 — inkl. des Rot-Fall-Hinweises in (c), der real bestätigt, dass der VORHER geplante `pruefbefehl` `FEHLER` statt `GRUEN` geliefert hätte; `npm run check` (775 Tests) und die AK2-Probe (echtes Wegwerf-Projekt in `%TEMP%`, außerhalb aller Repos) → `ergebnis:'GRUEN'`, `exit_code:0`, `dauer_ms:1480`. **F-703-Nachtrag (25.09.2026):** `node scripts/check-f34-product-coach.mjs` (q) und `check-f42-projekt-harness.mjs` (d) beide grün mit ECHTEN argv statt künstlicher Fixtures — für ai-workforce (`startvorlagen/ai-workforce.json`-argv) erscheint `npm run check muss danach grün sein.`, für ein neues Projekt (`schreibeStartvorlageUndProfil`-argv) `npm run check:template muss danach grün sein.`, für ein Fremd-argv mit Leerzeichen `"C:\Program Files\Fremdtool\tool.exe" --run muss danach grün sein.` — kein roher absoluter Pfad mehr im Auftragstext. |
| F42-Projekt-Harness-Gate (WS-2, löst F-685) | `scripts/check-f42-projekt-harness.mjs` (f), zusätzlich `src/architekt/architekt.test.ts` | `istStackOffen` (fehlende `CLAUDE.md`/Füllungs-Marker → `true`, gefüllte Zeile → `false`); `validiereErgebnisArchitektur`s dritter Parameter `stackOffen`: bei `true` muss `entscheidungen_mensch` mindestens einen Eintrag mit `kategorie:'stack'` tragen; `baueArchitektAuftragstext` hängt den Stack-Hinweis nur bei `stackOffen:true` an; Laufzeit-Durchsetzung real in `scripts/leitstand-server.mjs`s `leseArchitekturErgebnisAusLaufakte` (alle drei Aufrufer geben `istStackOffen(repoWurzel)` mit) | `istStackOffen` gegen ein Fixture-Verzeichnis mit `## 🏗️ Technischer Stack [FÜLLUNG]` → `true`; `validiereErgebnisArchitektur({...}, undefined, true)` mit nur einer `kategorie:'fachlich'`-Entscheidung (F-685-Reallauf-Muster: Architekt beantwortet nur Fachfragen) → Verstoß `"Stack offen, aber keine Entscheidung mit kategorie stack vorgelegt (F-685)"` | `istStackOffen(process.cwd())` gegen DIESES Repo (Stack bereits gefüllt) → `false`, real belegt; `validiereErgebnisArchitektur({...}, undefined, true)` MIT einer `kategorie:'stack'`-Entscheidung (mind. zwei Optionen) → `[]`; `stackOffen:false` (Default, weggelassen) bleibt für jede bestehende Ausgabe — mit und ohne `kategorie`-Feld — unverändert gültig (Rückwärtskompatibilität, `npm run check` mit allen 784 Tests grün) |
| Regel 1j Prüfkette unverändert + 1h-Pflicht pruefketten_pfade (F-713, F-735) | `src/workflow/index.ts`, `src/aenderungsuebersicht/index.ts`; Gate `scripts/check-fixpaket-f30-vorbedingungen.mjs` | Ein Ausführungslauf darf die Prüfkette, mit der er gemessen wird, nicht verändern (Default-Liste `STANDARD_PRUEFKETTEN_MUSTER` plus `pruefketten_pfade` der Startvorlage, bei Umbenennung alter und neuer Pfad); bei erfasster Stack-Entscheidung verlangt 1h ein nicht leeres `pruefketten_pfade` in der Startvorlage. Durchsetzungsgrad ERZWUNGEN (ARCHITECTURE.md §8: Rot und Grün laufen real im Gate). Bekannte Grenzen: Die Prüfkette läuft vom Stand der Arbeitskopie; Groß-/Kleinschreibung zählt; die Default-Liste gilt nur ab Repo-Wurzel. | Halt: (c) package.json-scripts verändert, (d) bestehendes Gate verändert, (h) Umbenennung aus scripts/check-* heraus, (i) biome.json verändert, (j) Fremdprojekt-Muster pyproject.toml verändert, (m) Lauf trägt pruefketten_pfade in die Startvorlage im Repo selbst ein, (l) Stack entschieden ohne pruefketten_pfade, (l2) mit leerer Liste | Kein Halt: (k) neue Datei unter einem gelisteten Muster (NEU-Ausnahme), (e) nur ein neues Gate; Gegenfall zu (l): `scripts/check-f42-projekt-harness.mjs` (h3) |
...
- 2026-09-25, F42-Projekt-Harness-Gate (WS-2, F-707, BUG P1): das WS-2-Gate
  (Zeile oben) prüfte bislang nur den reinen Builder
  (`baueArchitektAuftragstext`) und den Validator, NICHT den realen
  Aufrufer — `scripts/leitstand-server.mjs` (~Zeile 4160,
  `starteWorkflowSchritt`) rief `baueArchitektAuftragstext` ohne das vierte
  Argument `stackOffen` auf, sodass der Default `false` griff und
  `STACK_OFFEN_HINWEIS` den Architekten zur Laufzeit NIE erreichte, obwohl
  `leseArchitekturErgebnisAusLaufakte` bei offenem Stack bereits eine
  `kategorie:'stack'`-Entscheidung verlangte. Gefunden bei der WS-2-
  Verifikation, nicht vom Gate selbst. Neue Prüfungen `scripts/check-f39-
  architekt.mjs` (n3)/(n4) — realer Schrittstart-Pfad über einen zweiten,
  eigenen Server mit `repoWurzel` = Wegwerf-Verzeichnis OHNE `CLAUDE.md`
  (`istStackOffen` dafür `true`): Rot-Fall real reproduziert (die Zeile
  `auftragstext = baueArchitektAuftragstext(auftragVersion.daten.auftragstext, modus, capabilityAuszug)`
  ohne das vierte Argument wiederhergestellt → `node scripts/check-f39-
  architekt.mjs` → Befund `(n4) F-707: … sollte den Stack-Hinweis tragen,
  tat es aber nicht`); mit `istStackOffen(repoWurzel)` als viertem
  Argument (Korrektur) → `✓ (n4)`, Exit 0. Grün-Fall (n3): derselbe reale
  Pfad gegen DIESES Repo (Stack gefüllt, `repoWurzel` = `process.cwd()`,
...
- 2026-08-31, F4-Invocation-Policy-Gate, Vertrag `f4-invocation-policy`:
  AC8-Grep-Zielkonflikt real aufgetreten und aufgelöst. SCOPE.7 verlangt
  für `invocation-policy.test.ts` dasselbe Wegwerf-Git-Repo-Fixture-Muster
  wie `authorization-boundary.test.ts` (F3) — die Testdatei ruft dafür
  `execFileSync('git', ...)` auf und importiert `'node:child_process'`.
  Ein erster, ungefilterter Lauf des AC8-Grep gegen `src/invocation-
  policy/*.ts` (ohne Testdatei-Ausnahme) meldete genau einen Befund:
  `AC8: verbotenes Muster (child_process/spawn/exec/execSync) in
  src/invocation-policy/invocation-policy.test.ts gefunden` — ausgelöst
  durch die wörtliche Zeichenkette `child_process` im Import-Pfad
  `'node:child_process'`, nicht durch einen Aufruf des geprüften
  Werkzeugs selbst. `feature.md` AC8 verbietet wörtlich nur das Starten
  „eines Kindprozesses **des zu prüfenden Werkzeugs**" — die Testdatei
  startet nie das geprüfte Werkzeug (Claude Code), nur `git` für ihre
  eigene Fixture. Gate-Skript entsprechend auf die Produktionsdateien des
  Moduls beschränkt (`datei.endsWith('.test.ts')` ausgeschlossen), im
  Skript selbst begründet kommentiert. Nach der Änderung: `node
  scripts/check-f4-invocation-policy.mjs` → `✓ AC8: kein child_process/
  spawn/exec/execSync in den Produktionsdateien von
  src/invocation-policy/*.ts (Testfixture ausgenommen, siehe Kommentar) —
  F4 startet nie selbst einen Werkzeugprozess.`, `✓ Keine Befunde.`,
  Exit 0. `src/invocation-policy/index.ts`/`types.ts`/
  `verbotene-aufrufparameter.ts` (die drei Produktionsdateien) enthalten
  weiterhin keinen Treffer — real geprüft, nicht nur durch die
  Ausnahmeregel angenommen.

- 2026-08-31, F4-Invocation-Policy-Gate, Nachtrag vor Commit (Advisor-
  Befund): die vier Fixtures unter `schemas/examples/kontrollzustand-
  invocation-policy-*.json` wurden von keinem Gate gelesen oder geprüft —
  Abweichung vom etablierten F5-Muster (`check-f5-context-builder.mjs`
  Abschnitt „Payload-Fixture(s) geprüft"). Fix: `validiereBaselineEintrag`
  und `validiereWirksamkeitsnachweisEintrag` in `src/invocation-policy/
  index.ts` additiv exportiert (waren modulintern, gleiches additiv-
  Prinzip wie der F3-Export), Gate-Skript um einen neuen Abschnitt (a) vor
  dem bisherigen Bedingung-1-Test ergänzt, exaktes Muster wie F5: vier
  Fixtures laden, gegen den passenden Validator mit `sollGueltigSein:
  true/false` prüfen, Konsolenzeile `✓ N Payload-Fixture(s) geprüft.`.
  Kein Ajv, kein neues Dependency — Projekt validiert durchgehend
  handgeschrieben. Erster Lauf fand real 2 Befunde, keine Kosmetik: beide
  `*.valid.json`-Fixtures enthielten einen `hash`/`schutzskript_hashes[1]`-
  Wert mit nur 62 statt 64 Hex-Zeichen (`'c'.repeat(...)` beim ursprüng-
  lichen Anlegen der Fixture verzählt) — ohne echte Prüfung unbemerkt
  geblieben. Beide Dateien korrigiert (Länge programmatisch per `node -e`
  gesetzt und verifiziert, nicht durch Abzählen der Zeichenkette in der
  Anzeige, die bei diesem Muster optisch täuscht). Danach: `node
  scripts/check-f4-invocation-policy.mjs` → `✓ 4 Payload-Fixture(s)
  geprüft.`, `✓ Keine Befunde.`, Exit 0; `npm run check` und `npm run
  check:template` → je Exit 0, `tests 57, pass 57, fail 0` (Testzahl
...
- 2026-09-02, F7-Result-Evaluator-Gate, Baudurchgang
  (`state/tasks/f7-result-evaluator.md`): `klassifiziereLauf`
  (`src/result-evaluator/index.ts`, neu) gebaut, Gate-Skript neu
  (`scripts/check-f7-result-evaluator.mjs`), in beide Ketten in
  `package.json` eingehängt. `leseErgebnisobjekt`
  (`src/claude-code-gateway/index.ts`) additiv exportiert (F-062
  gelöst) — reine Sichtbarkeitsänderung, bestehende F6a-Tests
  unverändert grün. Der `tool_input`→Tokens-Adapter
  (`toolInputZuTokens`/`tokenisiereCommand`) tokenisiert `command` am
  Leerzeichen und entfernt danach je Token ein umschließendes
  Anführungszeichen — real gegen den konstruierten Einbettungsfall aus
  plan-v1 Abschnitt 8.4 verifiziert (`sh -c 'echo
  --dangerously-skip-permissions'` → `bypass_verdacht_anzahl: 1`), nicht
  nur gegen die beiden unquotierten TP-03d-Messfälle. AK4-Rot-Fall real
  über einen temporären Codeeingriff erzeugt (siehe Tabellenzeile oben),
  nicht nur behauptet. Reviewer-/QA-Pass (frischer Kontext, F-046)
  anschließend durchlaufen, beide „Freigegeben mit Hinweisen" — sechs
  Testfälle und ein Typ-Guard-Fix vor Commit nachgezogen (siehe
  Tabellenzeile oben, Nachtrag), vier Findings F-064 bis F-067 für die
  bewusst zurückgestellten Punkte angelegt. `npm run check:template`
  (nur Harness-Gates) und `npm run check` (inkl. Projekt-Tests) je Exit
  0, `tests 87, pass 87, fail 0`. `git status --short` nach dem Lauf
...
- 2026-09-26, Doku-Gate Prüfung 4 (Paar `docs/harness/HARNESS-CHANGELOG.md`
  → `docs/harness/HARNESS-LEARNING-STATE.md`, aktiviert mit F-740): Rot —
  temporär `| 2026-12-31 | Kalibrierung Rot-Fall |` an den Changelog
  angehängt → `npm run check` Exit 1, genau ein Befund: „docs/harness/HARNESS-CHANGELOG.md:
  jüngstes Datum 2026-12-31 ist neuer als "Stand dieser Fassung: 2026-09-26"
  in docs/harness/HARNESS-LEARNING-STATE.md:12 — Ziel-Datei nachziehen oder
  Marker aktualisieren“. Grün — Zeile zurückgenommen (jüngstes Changelog-Datum
  2026-09-26 = Marker) → Doku-Check „Keine Befunde“, `npm run check` Exit 0.
