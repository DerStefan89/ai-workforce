...
Maßnahme: Behoben — `laufId` wird vor dem Schreiben gegen die reale `sparring-<projektId>`-Kette geprüft (`sparringLaufExistiert`, neu in `scripts/leitstand/routen-sparring.mjs`, liest über `listeVersionen`/`herkunft.lauf_id`), `auftragId` gegen `auftrag-<auftragId>` (`ladeArtefaktVersion`, Muster `POST /api/auftraege/<id>/routen`) — beide Prüfungen VOR `registriereSparringAuftragZuordnung`, eine unbekannte ID liefert 404 statt den Verweis zu speichern. Rot-/Grünfälle in `scripts/check-f34-product-coach.mjs` Abschnitt (x).
Feature/Run: F34-Fixpaket-Nachtrag (fix/f34-sparring-verlauf), 23.09.2026; behoben im Sparring-Auftrag-Verknüpfungs-Auftrag (a0d04470-d4eb-4204-aec4-ad3ee21e2b91), 23.09.2026. Quelle: claude/f34-feature-gate.
