...
Maßnahme: Bei Gelegenheit `laufId` gegen die reale `sparring-<projektId>`-Kette (`listeVersionen`) und `auftragId` gegen `auftrag-<auftragId>` (`ladeArtefaktVersion`) prüfen, bevor die Zuordnung geschrieben wird — nicht in diesem Nachtrag behoben, nur erfasst (Auftrags-Vorgabe).
Feature/Run: F34-Fixpaket-Nachtrag (fix/f34-sparring-verlauf), 23.09.2026. Quelle: claude/f34-feature-gate.

**F-632** · `PROCESS_IMPROVEMENT` · P2 · offen
...
Maßnahme: Nicht in F39 WS-1 behoben (explizit außerhalb des Scopes — "Nicht in WS-1: hoch.json, Router, Leitstand"). Für F39 WS-2 vorgemerkt: ein Fortsetzungsweg, der eine am Schritt-Automaten vorbei eingetragene Entscheidung strukturiert (z. B. als zusätzliches, dem nächsten Schritt vorangestelltes Kontextelement, Muster F8 WS-2b `vorgaengerLaufId`) statt eines vollständigen Replans.
Feature/Run: F39-WS-1-Auftrag (Bestandsaufnahme), 23.09.2026. Quelle: claude/f39-ws1-architekt.

**F-633** · `TECH_DEBT` · P2 · offen
...
Maßnahme: Nicht in F39 WS-1 behoben (Bestandsaufnahme, kein Bauauftrag). Für F39 WS-2 vorgemerkt: `AuftragV0Daten` um ein optionales, strukturiertes Herkunftsfeld ergänzen (z. B. `herkunft: { art: 'projekt_interview' | 'sparring' | 'manuell', ... }`, Schema-Migration wie bei `Ressource.herkunft`), das der Router deterministisch vor seinem LLM-Urteil auswertet.
Feature/Run: F39-WS-1-Auftrag (Bestandsaufnahme), 23.09.2026. Quelle: claude/f39-ws1-architekt.
