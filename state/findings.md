...
Maßnahme: Vorschlag beider Reviewer — `baueAnzeigeListe('sparring')` analog zu `ladeRollenVerlaufsfenster` nach `(eintrag.modus ?? 'feature') === sparringUntermodus` filtern (das `modus`-Feld muss dafür zuerst in `leseAntwort`/`baueAnzeigeListe` durchgereicht werden); zusätzlich prüfen, ob lokale Fehler-Einträge (`lokaleEintraege.push`) denselben Filter sonst umgehen. Nicht in diesem Review-Pass behoben (reine Prüfrolle, keine Schreibrechte) — Entscheidung (Fix vs. dokumentierte Grenze) steht noch aus.
...
Maßnahme: Vorschlag QA — einen minimalen `auftragErstelltId`-Rückverweis im Lineage-Eintrag ergänzen, den `GET /api/sparring` projiziert und `renderAuftragBruecke` als "Auftrag bereits angelegt (→ #/projekt)"-Hinweis statt des nackten Triggers anzeigt. Nicht in diesem Review-Pass behoben (reine Prüfrolle) — Entscheidung (Fix vs. dokumentierte Grenze) steht noch aus.
Feature/Run: Feature-Review-Pass F34 (Gesamt), 23.09.2026. Quelle: claude/f34-feature-gate.

**F-626** · `PROCESS_GAP` · P3 · offen
Titel: Der Render-Nachweis (`features/F34/nachweis-ws3-ui/klickfolge.json`) deckt nur das Umschalten der Modus-/Untermodus-Buttons ab — keine einzige Nachricht wird gesendet, keiner der `art`-spezifischen Renderer und keine Auftrag-Brücke wird je real im Browser gerendert.
Beschreibung: QA-Pass F34 (Gesamt), 23.09.2026: die Klickfolge testet ausschließlich Hervorhebung, Sichtbarkeit und Reload-Persistenz der Umschalter-Buttons. Der in F-622 formulierte Anspruch des Render-Nachweises ("findet Bugs, die reine Quelltextprüfung nicht sieht") ist damit für den inhaltlich komplexesten Teil der WS-2/WS-3-UI (`renderAlternativen`/`renderScope`/`renderProjekt`, das Auftrag-Dialog-Formular) faktisch nicht eingelöst — genau der von der Aufgabenstellung genannte Pfad ("Projekt-Entwurf → Auftrag anlegen → zurück zu Jarvis") fehlt im Nachweis komplett.
Fundstelle: `features/F34/nachweis-ws3-ui/klickfolge.json` (`schritte`, ausschließlich Button-Klicks).
Auswirkung: Gering — betrifft nur die Vollständigkeit des Nachweises, nicht den geprüften Code selbst; ein realer Content-/Dialog-Bug in dem ungetesteten Bereich bliebe aber vom Render-Nachweis weiterhin unentdeckt.
Maßnahme: `render-nachweis`-Klickfolge in einer Folge-Iteration um einen Durchlauf mit echter Coach-Antwort (Nachrichteneingabe, Warten auf `art`-Antwort, Öffnen der Auftrag-Brücke) erweitern, bevor `npm run render-nachweis` als vollwertiger F-622-Ersatz für diese UI-Bereiche gilt.
Feature/Run: Feature-Review-Pass F34 (Gesamt), 23.09.2026. Quelle: claude/f34-feature-gate.
