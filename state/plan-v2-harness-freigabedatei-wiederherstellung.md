...
## TASK: harness-freigabedatei-wiederherstellung (v2, nach Advisor-Pass)

Dieser Plan ist Plan v1 (`state/plan-v1-harness-freigabedatei-wiederherstellung.md`)
plus vier Nachträge aus dem Advisor-Pass
(`state/advisor-findings-harness-freigabedatei-wiederherstellung.md`,
Urteil: Freigegeben mit Hinweisen). v1 bleibt unverändert stehen. Neue
oder geänderte Stellen gegenüber v1 sind mit `[v2]` markiert.
...
- [Fakt] `.claude/hooks/commit-guard.cjs` (aktueller Stand, 121 Zeilen)
  ...
  [v2, Advisor-Finding 1, entlastend bestätigt: Zeilenzahl, Aufgabenzahl
  und Fail-Closed-Verhalten stimmen exakt mit dem realen Code überein.]
...
```
  ...
  [v2, Advisor-Finding 4, offene Unsicherheit: Der Advisor konnte
  `git show f8d11f6^:.claude/hooks/commit-guard.js` mangels Git-Zugriff
  nicht selbst ziehen. Der Wortlaut ist durch drei unabhängige, bereits
  vorher committete Quellen inhaltlich stark gestützt
  (`docs/harness/programm-historie/harness-fix-5-commit-guard-haerten.md:34,58-64,138,181`
  und `state/gates.md`-Kalibrierungs-Log 17.08./23.08.2026), aber nicht
  byte-genau verifiziert. SCOPE 2 verlangt jetzt einen echten Diff vor
  Übernahme, siehe dort.]
  ...
  [v2, Advisor-Finding 3, entlastend bestätigt: Die Wortgrenzen-Erkennung
  für `gh`/`merge`, deren Stil hier übernommen werden soll, existiert
  exakt wie behauptet in `commit-guard.cjs:76-83`
  (`MERGE_GRENZE_VOR`/`MERGE_GRENZE_NACH`). Keine neue Abstraktion.]
  [v2, Advisor-Finding 5, MITTEL, bekanntes geerbtes Risiko, nicht Teil
  dieses Vertrags zu beheben: Die Pfadbildung `eingabe.cwd ||
  process.cwd()` ist bereits in
  `docs/harness/programm-historie/harness-fix-5-commit-guard-haerten.md:68-72`
  als offene, unbehobene `[Annahme]` dokumentiert — liegt das
  Arbeitsverzeichnis nicht auf der Repo-Wurzel, zeigt der Pfad ins Leere
  und der Hook meldet fälschlich "keine Freigabedatei". Dieser Plan
  reaktiviert diesen Code-Pfad unverändert (1:1-Übernahme laut Auftrag)
  und behebt die Schwäche NICHT — das wäre ein eigener, größerer Vertrag.
  Die Schwäche wird hier nur sichtbar gemacht und in
  `state/assumption-ledger.md` nachgetragen (SCOPE 2a), damit sie nicht
  erneut unbemerkt verschwindet.]
  [v2, Advisor-Finding 6, NIEDRIG, bekannte geerbte Grenze: TOCTOU-Fenster
  zwischen Zeitstempel-Prüfung und `fs.unlinkSync` (Einmalgebrauch) bei
  parallel laufenden git-Prozessen. Bereits in der historischen Fassung
  vorhanden, kein neuer Fehler dieses Plans. Für ein
  Solo-Maintainer-Repo mit einer aktiven Sitzung geringes Risiko — wird
  im Kopfkommentar des Hooks als bekannte Grenze benannt (SCOPE 2),
  nicht behoben.]
- [Fakt] `.claude/hooks/guard-settings.js` (aktueller Stand): `Edit|Write`-
  ...
  [v2, Advisor-Finding 2, entlastend bestätigt: Array-Inhalt und
  Matching-Logik stimmen exakt mit dem realen Code überein.]
- [Fakt, Befund dieser Sitzung, Option A entschieden] `state/
  ...
  [v2, Advisor-Finding 7, entlastend bestätigt: `tool_input.file_path`
  bei Edit/Write ist in der Praxis ein absoluter Pfad (belegt durch
  `state/gates.md:19`), der Guard matcht nicht auf `cwd` sondern auf
  diesen absoluten Pfad — robuster als der Bash-Freigabeprüfungspfad aus
  Finding 5. Option A funktioniert unabhängig vom Arbeitsverzeichnis der
  Sitzung.]
...
- [Fakt] Ziel-Laufzeitkonfiguration: `package.json` hat `"type":
  ...
  [v2, Advisor-Finding 9, Schlussfolgerung: Ein `require(...)`-Ladetest
  beweist "lädt fehlerfrei", nicht "die neuen Funktionen laufen unter
  dieser Node-Version korrekt" (kein Durchlauf durch `verarbeiten()` mit
  echten Daten). Deshalb verlangt SCOPE 8 jetzt zusätzlich, dass SCOPE 4-7
  auf DERSELBEN realen Maschine laufen wie der Smoke-Test — sonst bleibt
  eine Lücke zwischen Funktionsbeweis und Node-Versions-Beweis unsichtbar.]
...
2. `.claude/hooks/commit-guard.cjs`: dritte und vierte Aufgabe ergänzen,
   ...
   übernehmen. [v2] Kopfkommentar zusätzlich um die zwei bekannten,
   geerbten Grenzen aus Finding 5 (cwd-Abhängigkeit der Pfadbildung) und
   Finding 6 (TOCTOU-Fenster bei Einmalgebrauch) ergänzen — ein Satz je
   Grenze, nicht behoben, nur benannt.
   [v2] VOR der Übernahme des zitierten Wortlauts: `git show
   f8d11f6^:.claude/hooks/commit-guard.js` real ziehen und gegen das in
   Plan v2 zitierte CONTEXT diffen (Advisor-Finding 4). Weicht der reale
   historische Code vom Zitat ab: anhalten, Abweichung melden, NICHT
   stillschweigend den Zitat-Wortlaut übernehmen.
   [v2] `state/assumption-ledger.md`: Eintrag nachtragen für die
   cwd-Abhängigkeit aus Finding 5 (Pfadbildung `eingabe.cwd ||
   process.cwd()`, unbehoben, geerbt aus der historischen Fassung).
3. `.claude/hooks/guard-settings.js`: `GUARDED_FILES` um `state/
   ...
   werden, nicht vom Modell"). [v2] Neuer Eintrag trägt wie der bestehende
   Eintrag sowohl `path` als auch `suffix` (`state/freigabe-commit.md`
   bzw. `/state/freigabe-commit.md`), analog zum bestehenden
   `.claude/settings.json`-Eintrag (Advisor-Finding 7).
4. Rot-Fall Freigabedatei-Pflicht: `git commit` (Testfall, z. B. leerer
   ...
   verweigert. Wortlaut zeigen. [v2] Läuft auf derselben realen Maschine
   wie SCOPE 8 (siehe dort).
5. Grün-Fall Freigabedatei-Pflicht: Stefan legt außerhalb dieser Sitzung
   ...
   siehe NICHT. [v2] Läuft auf derselben realen Maschine wie SCOPE 8.
...
8. Lade-/Smoke-Test auf der REALEN Zielmaschine (nicht Sandbox/Bridge):
   ...
   [v2, Advisor-Finding 9/10] SCOPE 4-7 laufen auf DERSELBEN realen
   Maschine wie dieser Smoke-Test — im Bericht ausdrücklich festhalten,
   auf welcher Maschine (nativ, nicht über Dateibrücke/Sandbox) alle
   Schritte 4-8 tatsächlich ausgeführt wurden, analog zur N12-Vorgabe im
   B6-Vorgänger-Vertrag.
9. `state/gates.md`: neue Zeile(n) für Freigabedatei-Pflicht und
   ...
   [v2, Advisor-Finding 8] Zusätzlich: die BESTEHENDE Tabellenzeile zum
   `commit-guard.cjs`-Hook, die aktuell behauptet, die
   Freigabe-Datei-Pflicht sei "mit Befund B6 ersatzlos entfernt", auf den
   neuen Ist-Zustand korrigieren (Pflicht wiederhergestellt). Nur die
   Ist-Zustand-Tabellenzeile wird korrigiert — das Kalibrierungs-Log
   selbst (historisches Protokoll) bleibt unverändert stehen, analog N3
   im B6-Vorgänger-Vertrag.
...
- [v2] Die cwd-Abhängigkeit aus Finding 5 oder das TOCTOU-Fenster aus
  Finding 6 beheben — beide sind bekannte, geerbte Grenzen, die hier nur
  benannt, nicht repariert werden. Eine Reparatur wäre ein eigener
  Vertrag.
...
- [v2] Ausdrückliche Angabe, dass SCOPE 4-8 auf derselben realen Maschine
  liefen (welche, wie sichergestellt).
- [v2] Ergebnis des `git show f8d11f6^:...`-Diffs aus SCOPE 2 (Übereinstimmung
  bestätigt oder Abweichung gemeldet).
- Aktualisierte Zeilen und Log-Eintrag in `state/gates.md`, inklusive der
  korrigierten Bestands-Tabellenzeile aus SCOPE 9.
- Neuer Eintrag in `state/assumption-ledger.md` (SCOPE 2).
...
- [v2] Der `git show f8d11f6^:...`-Diff aus SCOPE 2 weicht vom in diesem
  Plan zitierten Wortlaut ab → anhalten, Abweichung zeigen, NICHT
  stillschweigend den Plan-Wortlaut übernehmen, auf Anweisung warten.
...
- [v2] Eine mögliche Behebung der cwd-Abhängigkeit (Finding 5) oder des
  TOCTOU-Fensters (Finding 6) ist ausdrücklich NICHT Teil dieses Vertrags.
