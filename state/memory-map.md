...
| Execution-Controller-Modul (F8 WS-1) (führt einen Lauf vollständig durch F5 `baueKontextpaket` → F6a `baueAufruf`/`starteGateway` → F7 `klassifiziereLauf` → F1B `stelleLaufstatusFest`, feste Reihenfolge, bricht bei F5/F6a-Ablehnung mit deren unverändertem Grund ab) | `src/execution-controller/` | kein Touch an F1B/F5/F6a/F7 (nur von außen aufgerufen, D1), keine eigene Prüf-/Klassifikationsregel (AK1/AK3-Gate), kein eigener `schreibeWirkungsmarke`-Aufruf für die Haupt-`laufId` (D5), WS-2 (E-186-Eskalation, erneuter Anlauf) noch nicht gebaut |
