...
| MCP-Werkzeuge im Ausführungslauf | `--strict-mcp-config`, `--mcp-config` (Default `'{"mcpServers":{}}'` in `baueAufruf`) | **ERZWUNGEN** | **gemessen** — Rot-/Grün-Fall für eine schreibende Rolle real gemessen (F31 WS-3c, `features/F31/nachweis-mcp-begrenzung.md`): `mcp_servers` geht von zwei Einträgen auf `[]`, Werkzeugliste von 13 auf 5 Einträge zurück |
...
`[Fakt]` Sieben von neun Punkten sind geschlossen (1, 2, 6, 7, 9 mit A1–A9; 4 mit E7; 3 mit F6b WS-F). Zwei bleiben an noch nicht ausgeführte Verträge gebunden (5, 8).
