...
| H3 | Manuelle Compaction wird blockiert, wenn der Zwischenstand fehlt oder älter als sechzig Minuten ist | `.claude/hooks/zwischenstand-pruefen.cjs`, PreCompact, `decision: "block"` | `[Fakt]` Nur bei `trigger === "manual"`. Bei automatischer Compaction nur `systemMessage` — also Selbstauskunft im entscheidenden Moment |
| H4 | Zwischenstand wird beim Sitzungsstart automatisch geladen | `.claude/hooks/zwischenstand-laden.cjs`, SessionStart | `[Fakt]` Matcher `startup\|resume\|compact\|fork` — `clear` fehlt. Nach `/clear` lädt nichts. Befund V1.1 |
...
| H6 | Erinnerung an Kontext-Hygiene alle dreißig Nachrichten | `.claude/hooks/session-reminder.cjs`, UserPromptSubmit | `[Schlussfolgerung]` Der Hook feuert zuverlässig, aber sein Ergebnis ist ein Hinweistext. Mechanisch ein HOOK, in der Wirkung eine Selbstauskunft mit Wecker |
